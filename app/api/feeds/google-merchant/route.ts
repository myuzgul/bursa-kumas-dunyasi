...
export async function GET() {
  try {
    ...
    <item>
      ...
      <g:description>${escapeXml(p.description || p.short_description || `${p.name} lüks kumaş.`)}</g:description>
      <g:link>${escapeXml(prodUrl)}</g:link>
      <g:image_link>${escapeXml(varImage)}</g:image_link>
      <g:brand>Bursa Kumaş Dünyası</g:brand>
      <g:condition>new</g:condition>
      <g:availability>${varInStock ? 'in_stock' : 'out_of_stock'}</g:availability>
...
<rss xmlns:g="http://base.google.com/ns/1.0" version="2.0">
  <channel>
    <title>Bursa Kumaş Dünyası - Google Alışveriş Ürün Kataloğu</title>
    <link>${siteUrl}</link>
    <description>Bursa Kumaş Dünyası güncel döşemelik ve perdelik kumaş ürün feed beslemesi.</description>
    ${itemsXml}
  </channel>
</rss>`;
    ...
    return new Response(`Error generating Google Merchant feed: ${error.message}`, {
      status: 500,
    });
  }
}
